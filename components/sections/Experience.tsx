...
const companyColors: Record<string, string> = {
  Vouched: "border-emerald-500 bg-emerald-500",
  Chainalysis: "border-blue-500 bg-blue-500",
  "Ekata (a Mastercard Company)": "border-purple-500 bg-purple-500",
};

function getCompanyColor(company: string) {
  return companyColors[company] || "border-primary bg-primary";
}
  ...
  return (
    <SectionWrapper id="experience">
      ...
      <div className="relative mx-auto max-w-3xl">
        ...
        >
          {groups.map((group, gi) => {
            const color = getCompanyColor(group.company);
            const dotColor = color.split(" ")[1]; // bg-xxx-500
            const borderColor = color.split(" ")[0]; // border-xxx-500
            ...
            return (
              ...
              >
                ...
                <div className="relative md:pl-12 mt-8 first:mt-0 mb-1">
                  ...
                  <div
                    className={`absolute left-[11px] top-[6px] hidden h-[17px] w-[17px] items-center justify-center rounded-full border-2 ${borderColor} bg-background md:flex`}
                  >
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <div className={`hidden h-px flex-1 md:block ${dotColor}/20`} />
                  </div>
                </div>
                ...
                {group.roles.map((role, ri) => (
                  ...
                  >
                    ...
                    {group.roles.length > 1 && (
                      <div
                        className={`absolute left-[19px] top-0 hidden h-full w-px md:block ${dotColor}/30`}
                      />
                    )}

                    {/* Small role dot */}
                    <div
                      className={`absolute left-[15px] top-[24px] hidden h-[9px] w-[9px] rounded-full ${dotColor}/60 md:block`}
                    />

                    <AccordionItem
                      value={role.company + role.period}
                      className={`border-b border-border/50 ${ri === 0 ? "" : ""}`}
                    >
                        ...
                        <ul className="space-y-2 pb-2 text-sm leading-relaxed text-muted-foreground">
                          {role.bullets.map((bullet, j) => (
                            <li key={j} className="flex gap-2">
                              <span
                                className={`mt-1.5 h-1.5 w-1.5 flex-shrink-0 rounded-full ${dotColor}/60`}
                              />
                              {bullet}
                            </li>
                          ))}
